...
const SelectedProject = () => {
	...
	useEffect(() => {
		if (!reduceMotion) {
			gsap.fromTo('#selectedProjectElement', {
				translateX: '-=50', autoAlpha: 0, scale: .8,
			}, {
				translateX: 0, scale: 1, autoAlpha: 1, duration: .4, ease: 'back', stagger: .1,
			})
		}
	}, [reduceMotion])
			...
			{project && <main className={styles.mainContainer}>
				<section className={styles.projectContent}>
					<div className={styles.heading}>
						<p id={'selectedProjectElement'}>{project.type}</p>
						<h1 id={'selectedProjectElement'}>{project.name}</h1>
						<div className={styles.links}>
							<p id={'selectedProjectElement'}>Links</p>

							<div className={styles.row} id={'selectedProjectElement'}>
								{project.links.map(({ name, url, icon }) => (
					...
					<div className={styles.projectDetails}>
						...
						<div className={styles.projectSecondDetails}>
							<p id={'selectedProjectElement'}>Date</p>
							<p id={'selectedProjectElement'}>{project.date}</p>
						</div>

						<div className={styles.projectSecondDetails}>
							<p id={'selectedProjectElement'}>Role</p>
							<p id={'selectedProjectElement'}>{project.role}</p>
						</div>
					</div>
				</section>
